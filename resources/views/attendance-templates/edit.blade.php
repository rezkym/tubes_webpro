...
@section('title', 'Edit Attendance Template')

@section('content')
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3">Edit Attendance Template</h1>
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('attendance-templates.update', $attendance_template) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                name="name" value="{{ old('name', $attendance_template->name) }}" required>
                            @error('name')
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('day') is-invalid @enderror" id="day" name="day"
                                ...
                                @foreach ($days as $value => $label)
                                    <option value="{{ $value }}"
                                        {{ old('day', $attendance_template->day) == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="time" class="form-control @error('start_time') is-invalid @enderror"
                                ...
                                value="{{ old('start_time', $attendance_template->start_time->format('H:i')) }}" required>
                            @error('start_time')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="time" class="form-control @error('end_time') is-invalid @enderror"
                                ...
                                value="{{ old('end_time', $attendance_template->end_time->format('H:i')) }}" required>
                            @error('end_time')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select class="form-select @error('school_class_id') is-invalid @enderror" id="school_class_id"
                                ...
                                @foreach ($classes as $class)
                                    <option value="{{ $class->id }}"
                                        {{ old('school_class_id', $attendance_template->school_class_id) == $class->id ? 'selected' : '' }}>
                                        {{ $class->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-select @error('school_subject_id') is-invalid @enderror"
                                ...
                                @foreach ($subjects as $subject)
                                    <option value="{{ $subject->id }}"
                                        {{ old('school_subject_id', $attendance_template->school_subject_id) == $subject->id ? 'selected' : '' }}>
                                        {{ $subject->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-select @error('teacher_profile_id') is-invalid @enderror"
                                ...
                                @foreach (App\Models\TeacherProfile::with('user')->get() as $teacher)
                                    <option value="{{ $teacher->id }}"
                                        {{ old('teacher_profile_id', $attendance_template->teacher_profile_id) == $teacher->id ? 'selected' : '' }}>
                                        {{ $teacher->full_name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                            rows="3">{{ old('description', $attendance_template->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <div class="form-check">
                            <input type="checkbox" class="form-check-input @error('is_active') is-invalid @enderror"
                                ...
                                {{ old('is_active', $attendance_template->is_active) ? 'checked' : '' }}>
